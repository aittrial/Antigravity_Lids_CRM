...
def main():
    ...
    if choice == "📊 Аналитика":
        ...
        if not df_all.empty:
            ...
            m_w[1].metric("🔵 Работа", len(df_week[df_week['status_color']=='blue']))
            m_w[2].metric("🟡 Ждут", len(df_week[df_week['status_color']=='yellow']))
            m_w[3].metric("🔴 Отказ", len(df_week[df_week['status_color']=='red']))
            m_w[4].metric("🟢 Возврат", len(df_week[df_week['status_color']=='green']))
            m_w[5].metric("🟣 Офис", len(df_week[df_week['status_color']=='purple']))
            m_w[6].metric("💗 Работа 2", len(df_week[df_week['status_color']=='pink']))
    ...
    # --- База данных ---
        ...
        with c2:
            ...
            if user_role == "superadmin" and st.button("📦 ВСЁ В АРХИВ"): set_archive_threshold(); st.rerun()
        with c3:
            st.subheader("🔥 Очистка")
            if user_role == "superadmin" and st.button("🔥 УДАЛИТЬ ВСЁ"): clear_all_leads(); st.rerun()
    ...
    # --- Администрирование ---
        ...
        new_m = c_m.text_input("Email:")
        new_r = c_r.selectbox("Роль", ["admin", "analyst"])
        if st.button("Добавить доступ"):
            add_allowed_email(new_m, new_r); st.success(f"Добавлен {new_r}"); st.rerun()
        st.divider()
        for d in get_allowed_emails():
            col1, col2 = st.columns([4, 1])
            col1.write(f"• {d['email']} ({d['role']})")
            if col2.button("Удалить", key=f"del_{d['email']}"): delete_allowed_email(d['email']); st.rerun()
