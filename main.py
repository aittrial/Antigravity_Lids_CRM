...
def render_leads_list(leads_data, start_order=1, can_archive=False):
    """Явная отрисовка каждой карточки лида."""
    if not leads_data:
    ...
    for i, row in enumerate(leads_data):
        ...
        with st.expander("🛠 Управление"):
            ...
            st.divider()
            # Поля редактирования (развернуто)
            ...
            if st.button("💾 Сохранить", key=f"save_{row['id']}"):
                update_lead(row['id'], full_name=new_fn, phone=new_ph, status_color=new_sc)
...
def main():
    ...
    if choice == "📊 Аналитика":
        st.header("📊 Аналитика")
        leads_all = get_leads(mode="all")
        df_all = pd.DataFrame(leads_all)
        
        if not df_all.empty:
            # 1. СТАТИСТИКА ЗА ВСЕ ВРЕМЯ (Исправлен дубликат)
        ...
        s_query = col_f1.text_input("🔍 Поиск")
        d_range = col_f2.date_input("📅 Даты", value=(date.today()-timedelta(days=30), date.today()))
        c_filt = col_f3.selectbox("🎨 Статус", FILTER_COLOR_MAP)
        src_filt = col_f4.selectbox("📡 Источник", FILTER_SOURCE_MAP)
        
        st_d, en_d = (d_range[0], d_range[1]) if len(d_range) == 2 else (None, None)
        
        tab_active, tab_archive = st.tabs(["🔥 Активные", "📦 Архив"])
        with tab_active:
            data_active = get_leads(s_query, st_d, en_d, mode="active", status_filter=c_filt, source_filter=src_filt)
            render_leads_list(data_active, can_archive=True)
        with tab_archive:
            # ЗДЕСЬ ТЕПЕРЬ ПОДГРУЖАЕТСЯ ВЕСЬ АРХИВ
            data_archive = get_leads(s_query, st_d, en_d, mode="archive", status_filter=c_filt, source_filter=src_filt)
            render_leads_list(data_archive)

    elif choice == "➕ Новый лид":
        st.header("➕ Новый лид")
        with st.form("form_new_lead"):
        ...
        if user_role == "superadmin":
            if st.button("📦 ВСЕ ЛИДЫ В АРХИВ"):
                set_archive_threshold()
                st.rerun()
            st.divider()
            confirm = st.checkbox("Я подтверждаю полную очистку базы")
            if confirm:
        ...
        new_r = st.selectbox("Роль:", ["admin", "analyst"])
        if st.button("Добавить"):
            add_allowed_email(new_m, new_r)
            st.rerun()
        ...
        for e in emails:
            ...
            col_e1.write(f"• {e['email']} ({e['role']})")
            if col_e2.button("Удалить", key=f"del_{e['email']}"):
                delete_allowed_email(e['email'])
                st.rerun()
