...
def main():
    ...
    if choice == "📊 Аналитика":
        st.header("📊 Аналитика")
        df_all = pd.DataFrame(get_leads(mode="all")) # Здесь нет лимита!
        if not df_all.empty:
            ...
            m2 = st.columns(7)
            m2[0].metric("Новых (7д)", len(df_week)); m2[1].metric("🔵", len(df_week[df_week['status_color']=='blue'])); m2[2].metric("🟡", len(df_week[df_week['status_color']=='yellow'])); m2[3].metric("🔴", len(df_week[df_week['status_color']=='red'])); m2[4].metric("🟢", len(df_week[df_week['status_color']=='green'])); m2[5].metric("🟣", len(df_week[df_week['status_color']=='purple'])); m2[6].metric("💗", len(df_week[df_week['status_color']=='pink']))
        ...
        with tab_archive:
            ...
            with nav1:
                if st.button("⬅️ Назад", key="btn_arc_prev"):
                    if st.session_state.archive_page_number > 0:
                        st.session_state.archive_page_number -= 1
                        st.rerun()
            with nav2:
                st.markdown(f"<center>Страница {st.session_state.archive_page_number + 1}</center>", unsafe_allow_html=True)
            with nav3:
                # ВАЖНО: Кнопка «Вперед» активна, если на странице 50 лидов
                if len(data_archive) == leads_per_page:
                    if st.button("Вперед ➡️", key="btn_arc_next"):
                        st.session_state.archive_page_number += 1
                        st.rerun()
                else:
                    st.write("Конец")
        ...
        with c1:
            ...
            if all_raw:
                ...
                st.download_button("📥 Скачать Excel (.xlsx)", data=towrite.getvalue(), file_name=f"export_{date.today()}.xlsx")
        ...
        with c3:
            ...
            if user_role == "superadmin":
                if st.checkbox("Я подтверждаю полную очистку"):
        ...
        if st.button("Добавить доступ"):
            if nm: add_allowed_email(nm, nr); st.rerun()
        st.divider()
