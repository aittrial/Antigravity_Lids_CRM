...
def render_leads_list(leads_data, start_order=1, can_archive=False):
    ...
    for i, row in enumerate(leads_data):
        ...
        with st.expander("Управление"):
            ...
            un, up, ue = c1.text_input("ФИО", row['full_name'], key=f"n_{row['id']}"), c2.text_input("Тел", row['phone'], key=f"p_{row['id']}"), c3.text_input("Email", row['email'], key=f"e_{row['id']}")
            ...
            curr_c = c4.text_input("Уточните курс", row['course_name'], key=f"cm_{row['id']}") if c_sel == "Other" else c_sel
            ut, us = c5.text_input("Время", row.get('preferred_time',''), key=f"t_{row['id']}"), c6.selectbox("Статус", COLOR_KEYS, index=COLOR_KEYS.index(row['status_color']), key=f"s_{row['id']}")
            c7, c8 = st.columns(2)
            ...
            ucom = c8.text_area("Комментарий", row['comment'] or "", key=f"com_{row['id']}", height=100)
            if st.button("💾 Сохранить", key=f"sv_{row['id']}"):
                update_lead(row['id'], full_name=un, phone=up, email=ue, course_name=curr_c, preferred_time=ut, status_color=us, comment=ucom, source=curr_src); st.rerun()
    ...
    # --- Новый лид ---
        ...
        with st.form("new_lead_form", clear_on_submit=True):
            ...
            if st.form_submit_button("Создать"):
                if fn and ph: add_lead(fn, ph, em, crs, tm, src, cmm, stt); st.success("Добавлено!"); st.rerun()
